...
class Drawing extends import_editor.StateNode {
  ...
  startShape() {
    ...
    if (this.initialShape) {
      ...
      if (shape2 && this.segmentMode === "straight" && this.editor.isShapeInPage(shape2)) {
  ...
  updateDrawingShape() {
    ...
    switch (this.segmentMode) {
      case "starting_straight": {
        ...
        const hasMovedFarEnough = import_editor.Vec.Dist2(pagePointWhereNextSegmentChanged, inputs.getCurrentPagePoint()) * zoom * zoom > this.editor.options.dragDistanceSquared;
      ...
      case "free": {
        ...
        if (cachedPoints.length > this.util.options.maxPointsPerShape) {
          ...
          const initialPoint = new import_editor.Vec(0, 0, this.isPenOrStylus ? +(z * 1.25).toFixed(2) : 0.5);
          this.currentSegmentPoints = [initialPoint];
          this.editor.createShape({
            id: newShapeId,
            type: this.shapeType,
  ...
  getLineLength(segments) {
    ...
    for (let j = 0; j < segments.length; j++) {
      ...
      for (let i = 0; i < points.length - 1; i++) {
        length += import_editor.Vec.Dist(points[i], points[i + 1]);
      }
    }
    return length;
  }
