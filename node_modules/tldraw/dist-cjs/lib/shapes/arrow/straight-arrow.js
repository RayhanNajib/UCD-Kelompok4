...
function getStraightArrowInfo(editor, shape, bindings, arrowStrokeWidth) {
  ...
  if (!isSelfIntersection) {
    if (relationship !== "start-contains-end" && startShapeInfo && arrowheadStart !== "none" && !startShapeInfo.isExact) {
      strokeOffsetA = arrowSW / 2 + ("size" in startShapeInfo.shape.props ? theme.strokeWidth * (import_default_shape_constants.STROKE_SIZES[startShapeInfo.shape.props.size] ?? 0) / 2 : 0);
    ...
    if (relationship !== "end-contains-start" && endShapeInfo && arrowheadEnd !== "none" && !endShapeInfo.isExact) {
      strokeOffsetB = arrowSW / 2 + ("size" in endShapeInfo.shape.props ? theme.strokeWidth * (import_default_shape_constants.STROKE_SIZES[endShapeInfo.shape.props.size] ?? 0) / 2 : 0);
