...
var import_editors = require("../globals/editors");
...
class Editor extends import_eventemitter3.default {
  ...
  constructor({
    ...
    this.on("mount", () => {
      ...
      (0, import_editors.registerMountedEditor)(this);
    });
    this.on("unmount", () => {
      this._isMounted.set(false);
      (0, import_editors.unregisterMountedEditor)(this);
    });
    ...
    if (this.store.props.collaboration?.mode) {
      ...
      this.disposables.add(
        (0, import_state.react)("update collaboration mode", () => {
          ...
          (0, import_state.unsafe__withoutCapture)(
  ...
  getShapesRotatedPageBounds(shapeIds) {
    ...
    const boxFromRotatedVertices = import_Box.Box.FromPoints(
      shapeIds.flatMap(
        (id) => this.getShapePageTransform(id).applyToPoints(this.getShapeGeometry(id).bounds.corners)
  ...
  setErasingShapes(shapes) {
    ...
    this.run(
      () => {
        if (!(0, import_utils.areArraysShallowEqual)(ids, erasingShapeIds)) {
  ...
  _getFitZoom(fit) {
    ...
    switch (fit) {
      ...
      default: {
        throw (0, import_utils.exhaustiveSwitchError)(fit);
      }
    }
  }
      ...
      if (cameraOptions.constraints) {
        ...
        if (opts?.reset) {
          ...
          switch (behaviorX) {
            ...
            case "inside": {
              if (z < zx) x = (0, import_utils2.clamp)(x, minX, (vsb.w - px) / z - bounds.w - bounds.x);
              else x = (0, import_utils2.clamp)(x, minX + freeW, minX);
              break;
            }
            case "outside": {
              x = (0, import_utils2.clamp)(x, px / z - bounds.w - bounds.x, (vsb.w - px) / z - bounds.x);
              break;
            }
            case "free": {
              break;
          ...
          switch (behaviorY) {
            ...
            case "inside": {
              if (z < zy) y = (0, import_utils2.clamp)(y, minY, (vsb.h - py) / z - bounds.h - bounds.y);
              else y = (0, import_utils2.clamp)(y, minY + freeH, minY);
              break;
            }
            case "outside": {
              y = (0, import_utils2.clamp)(y, py / z - bounds.h - bounds.y, (vsb.h - py) / z - bounds.y);
              break;
            }
            case "free": {
              break;
  ...
  _animateViewport(ms) {
    ...
    const left = (0, import_utils.lerp)(start.minX, end.minX, t);
    const top = (0, import_utils.lerp)(start.minY, end.minY, t);
    const right = (0, import_utils.lerp)(start.maxX, end.maxX, t);
  ...
  _getShapeMaskedPageBoundsCache() {
    return this.store.createComputedCache("shapeMaskedPageBoundsCache", (shape) => {
      ...
      if (pageMask) {
        ...
        if (pageMask.length === corners.length && corners.every((p, i) => import_Vec.Vec.Equals(p, pageMask[i]))) {
  ...
  getShapeAtPoint(point, opts = {}) {
    ...
    for (let i = shapesToCheck.length - 1; i >= 0; i--) {
      ...
      if (isGroup && (isShapeFrameLike || (this.isShapeOfType(shape, "note") || this.isShapeOfType(shape, "arrow") || this.isShapeOfType(shape, "geo") && shape.props.fill === "none") && shapeUtil.getText(shape)?.trim())) {
  ...
  getShapeAndDescendantIds(ids) {
    ...
    for (const shape of (0, import_utils.compact)(ids.map((id) => this.getShape(id))).sort(import_utils.sortByIndex)) {
  ...
  getDraggingOverShape(point, droppingShapes) {
    ...
    const maybeDraggingOverShapes = this.getShapesAtPoint(point, {
      ...
      (s) => !droppingShapes.includes(s) && !s.isLocked && !this.isShapeHidden(s) && !excludedIds.has(s.id)
    );
      ...
      const { shapesToCreateWithOriginals, bindingsToCreate } = withIsolatedShapes(
        ...
        (bindingIdsToMaintain) => {
          ...
          for (const originalId of orderedShapeIds) {
            ...
            let ox = 0;
            let oy = 0;
            if (offset && initialIds.has(originalId) && !shapeIdSet.has(originalShape.parentId)) {
  ...
  getShapeClusters(shapes, type, opts) {
    ...
    if (opts?.filterAxisAligned) {
      freshShapes = freshShapes.filter((s) => {
        const remainder = Math.abs(this.getShapePageTransform(s).rotation() % import_utils2.HALF_PI);
        return Math.min(remainder, import_utils2.HALF_PI - remainder) < 1e-9;
      });
    }
    const clusters = [];
    const allBounds = [];
  ...
  packShapes(shapes, _gap) {
    ...
    shapeClustersToPack.sort((a, b) => b.pageBounds.width - a.pageBounds.width).sort((a, b) => b.pageBounds.height - a.pageBounds.height);
  ...
  animateShapes(partials, opts = { animation: import_constants.DEFAULT_ANIMATION_OPTIONS }) {
    ...
    const handleTick = (elapsed) => {
      ...
      for (let i = 0, n = animations.length; i < n; i++) {
        ...
        updates.push({
          ...end,
          x: (0, import_utils.lerp)(start.x, end.x, t),
          y: (0, import_utils.lerp)(start.y, end.y, t),
          opacity: (0, import_utils.lerp)(start.opacity, end.opacity, t),
          rotation: (0, import_utils.lerp)(start.rotation, end.rotation, t),
  ...
  _getSelectionSharedStyles() {
    ...
    for (const shape of this._getStyleableShapes(this.getSelectedShapes())) {
      for (const [style, propKey] of this.styleProps[shape.type]) {
        sharedStyles.applyValue(style, (0, import_utils.getOwnProperty)(shape.props, propKey));
      }
    }
    return sharedStyles;
  }
  ...
  putContentOntoCurrentPage(content, opts = {}) {
    ...
    for (const asset of assets) {
      ...
      if (asset.type === "image" && asset.props.src?.startsWith("data:image") || asset.type === "video" && asset.props.src?.startsWith("data:video")) {
        ...
        const assetWithoutSrc = (0, import_utils.structuredClone)(asset);
...
__decorateClass([
  import_utils.bind
], Editor.prototype, "_releaseShiftKey", 1);
__decorateClass([
  import_utils.bind
], Editor.prototype, "_releaseAltKey", 1);
__decorateClass([
  import_utils.bind
], Editor.prototype, "_releaseCtrlKey", 1);
__decorateClass([
  import_utils.bind
], Editor.prototype, "_releaseMetaKey", 1);
