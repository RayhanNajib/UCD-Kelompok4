...
const ACTIVITY_TIMESTAMP_THROTTLE_MS = 1e3;
...
class InputsManager extends import_EditorManager.EditorManager {
  constructor(editor) {
    ...
    this.register(
      (0, import_state.react)("presence activity listeners", () => {
  ...
  _detachActivityListenersFn = null;
  /**
  ...
  _throttledActivityStamp = (0, import_utils.throttle)(
    () => {
      const pointer = this.editor.store.unsafeGetWithoutCapture(import_tlschema.TLPOINTER_ID);
      if (!pointer) return;
      this.editor.run(
        () => {
          ...
          this.editor.store.put([
            {
              ...
              // count as activity.
              lastActivityTimestamp: this.editor.store.unsafeGetWithoutCapture(import_tlschema.TLPOINTER_ID)?.lastActivityTimestamp ?? 0,
              meta: {}
            }
          ]);
        },
...
__decorateClass([
  import_utils.bind
], InputsManager.prototype, "markActivity", 1);
